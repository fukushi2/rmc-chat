...
fu.listen = function (port, host) {
  ...
  sys.puts("fu:Server at http://" + (host || "127.0.0.1") + ":" + port.toString() + "/");
  sys.puts("-----faiofjfioajjaoi");
};
...
fu.staticHandler = function (filename) {
  ...
  function loadResponseData(callback) {
    ...
    sys.puts("fu:loading " + filename + "...");
    readFile(filename, function (err, data) {
      if (err) {
        sys.puts("fu:Error loading " + filename);
      } else {
        body = data;
        ...
        sys.puts("fu:static file " + filename + " loaded");
        callback();
      }
    });
  }
